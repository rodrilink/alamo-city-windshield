...
- **THREE CRITICAL defects found by code review of Phase 06 — independently confirmed, NOT yet fixed.** See `06-REVIEW.md`. All three are latent (invisible at current data volume and on a Chicago-set dev machine) and will surface silently in production on Vercel:
  ...
  Note: CR-01's cleanest fix is a Postgres RPC doing `count(distinct session_id)` server-side (`SECURITY INVOKER` to preserve RLS) — a schema change. The TypeScript fallback (explicit `.order()` + `.limit()` + pagination) is more code and does not fix CR-02.
